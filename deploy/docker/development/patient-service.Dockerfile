...
RUN --mount=type=cache,target=/root/.cache/go-build \
    ...
    CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o /app/build/meeting-reminder-dispatcher ./cmd/meeting-reminder-dispatcher
...
COPY --from=builder /app/build/meeting-reminder-dispatcher ./build/meeting-reminder-dispatcher
