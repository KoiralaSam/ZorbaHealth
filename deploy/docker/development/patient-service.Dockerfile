...
RUN --mount=type=cache,target=/root/.cache/go-build \
    ...
    CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o /app/build/cron-dispatcher ./cmd/cron-dispatcher
...
COPY --from=builder /app/build/cron-dispatcher ./build/cron-dispatcher
