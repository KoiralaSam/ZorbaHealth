...
const getPlatformHospitalBreakdown = `-- name: GetPlatformHospitalBreakdown :many
...
WHERE h.active = true
ORDER BY
  CASE WHEN $1::text = 'calls' THEN COALESCE(cc.call_count, 0) END DESC,
  CASE WHEN $1::text = 'patients' THEN COALESCE(pc.patient_count, 0) END DESC,
  CASE WHEN $1::text = 'emergencies' THEN COALESCE(cc.emergency_count, 0) END DESC,
  h.name ASC
LIMIT $2
`

type GetPlatformHospitalBreakdownParams struct {
	SortBy      string `json:"sort_by"`
	ResultLimit int32  `json:"result_limit"`
}

type GetPlatformHospitalBreakdownRow struct {
...
func (q *Queries) GetPlatformHospitalBreakdown(ctx context.Context, arg GetPlatformHospitalBreakdownParams) ([]GetPlatformHospitalBreakdownRow, error) {
	rows, err := q.db.Query(ctx, getPlatformHospitalBreakdown, arg.SortBy, arg.ResultLimit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []GetPlatformHospitalBreakdownRow{}
	for rows.Next() {
		var i GetPlatformHospitalBreakdownRow
		if err := rows.Scan(
			&i.HospitalID,
			&i.HospitalName,
			&i.PatientCount,
...
type GetPlatformSummaryRow struct {
	TotalHospitals      int32   `json:"total_hospitals"`
	TotalPatients       int32   `json:"total_patients"`
	TotalCalls30d       int32   `json:"total_calls_30d"`
	TotalEmergencies30d int32   `json:"total_emergencies_30d"`
	AvgCallDurationSec  float64 `json:"avg_call_duration_sec"`
	ActiveHospitals7d   int32   `json:"active_hospitals_7d"`
}
...
func (q *Queries) GetPlatformSummary(ctx context.Context) (GetPlatformSummaryRow, error) {
	...
	err := row.Scan(
		...
		&i.TotalPatients,
		&i.TotalCalls30d,
		&i.TotalEmergencies30d,
		&i.AvgCallDurationSec,
		&i.ActiveHospitals7d,
	)
	return i, err
}
